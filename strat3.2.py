...
class MapNetwork:
    ...
    def nexus(self):
        ...
        for node in nodes_with_none_owner:
            ...
            score = -1 * surrounding_low_link_count + 2 * surrounding_none_owner_count + 2*node_degree - len(bridges) - continent_size/2
...
def handle_attack(game: Game, bot_state: BotState, query: QueryAttack, mapNetwork: MapNetwork) -> Union[MoveAttack, MoveAttackPass]:
    ...
    #attack strongest territory nearby
    def attack_strongest(territories: list[int]) -> Optional[MoveAttack]:
        ...
        for candidate_target in territories:
            ...
            #get the nearby stronest border
    ...
    if avg <=30 or game_state <500 or domination < 4:
        ...
        for base, percent in all_my_continents:
            if base in ['AS','EU']:
                pass
            elif 0<percent<1: #if continent is not fully owned
                for territory in strongest_territories:
                    adj = mapNetwork.get_neighbors(territory)
                    for i in adj:
                        if mapNetwork.G.nodes[i]['group'] == base:
                            move = attack_weakest(list(set(game.state.map.get_adjacent_to(territory)) - set(my_territories)))
                            if move != None:
                                return move
        #attack the weakest player in the game

        #attack the strongest player in the game if weak
        #attack weakest if strong
        ...
        for strongest, power in strongest_list[:-2]:
        ...
        for base, percent in all_my_continents[:-4]:
            if 0<percent<1: #if continent is not fully owned
                for territory in strongest_territories:
                    adj = mapNetwork.get_neighbors(territory)
                    for i in adj:
                        if mapNetwork.G.nodes[i]['group'] == base:
        ...
        if my_base_percent!=1: #continent not conquered
            for i in enemies_around_base:
                ...
                for node, troops in stationed_troops:
                    if node in adj_nodes and troops*2 <= enemy_troops and total_troops > (enemy_troops-troops)+1:
                        distributions[node] += (enemy_troops-troops)+1
                        remaining_troops -= (enemy_troops-troops)+1
                    break
                break
