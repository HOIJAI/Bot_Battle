...
def handle_claim_territory(game: Game, bot_state: BotState, query: QueryClaimTerritory) -> MoveClaimTerritory:
    ...
    gdpoints = {14, 35, 13, 1, 23, 34, 41, 32} #good poking points

    NA = {0,1,2,3,4,5,6,7,8}
    SA = {30,31,29,26}
    EU = {10,12,9,11,15,13,14}
    AF = {36,34,32,33,37,35}
    AS = {26,16,22,25,17,18,27,19,23,24,21,20}
    AU = {41,40,39,38}
    continent = [NA, SA, AF, AU, EU, AS]
    ...
    #set main base in preferred positions
    if len(unclaimed_territories) >=27:
        for i in continent:
            for j in i:
                if j in unclaimed_territories:
                    selected_territory = j
                    break

    elif 12<=len(unclaimed_territories)<27:
        selected_territory = next((i for i in gdpoints if i in unclaimed_territories), None)
        if selected_territory is None:
            selected_territory = max(unclaimed_territories, key=lambda x: len(game.state.map.get_adjacent_to(x)))
...
def handle_distribute_troops(game: Game, bot_state: BotState, query: QueryDistributeTroops) -> MoveDistributeTroops:
    ...
    # Define the sets for each continent
    NA = {0, 1, 2, 3, 4, 5, 6, 7, 8}
    SA = {30, 31, 29, 26}
    EU = {10, 12, 9, 11, 15, 13, 14}
    AF = {36, 34, 32, 33, 37, 35}
    AS = {26, 16, 22, 25, 17, 18, 27, 19, 23, 24, 21, 20}
    AU = {41, 40, 39, 38}
    continents = [NA, SA, AF, AU, EU, AS]

    # Get the territories owned by the player
    my_territories = game.state.get_territories_owned_by(game.state.me.player_id)

    # Function to locate the base continent to reinforce borders and conquer territories
    def locate_base(): 
        # Dictionary to count the territories owned in each continent
        count = {i: 0 for i in range(len(continents))}

        # Count the territories owned in each continent
        for t in my_territories:
            for j in range(len(continents)):
                if t in continents[j]:
                    count[j] += 1

        # Find the continent with the most owned territories but not fully owned
        max_count = 0
        max_continent = 0

        for i in range(len(continents)):
            continent_size = len(continents[i])
            if 0 < count[i] < continent_size and count[i] > max_count:
                max_count = count[i]
                max_continent = i

        return max_continent

    # Get the index of the base continent
    base_index = locate_base()
    # Get the territories in the base continent owned by the player
    base_territories = list(continents[base_index] & set(my_territories))

    # We will fortify and expand base territory,
    ...
    border_territories = game.state.get_all_border_territories(base_territories)
    other_territories = list(set(my_territories)-set(base_territories))
    ...
    if len(game.state.recording) < random.randint(2000,4000):
        troops_per_territory = int(total_troops*0.8) // len(border_territories)
        ...
        for player in weakest_players:
            ...
            if len(bordering_enemy_territories) > 0:
                ...
                increment = int(remaining*0.8)
                distributions[selected_territory] += increment
                remaining -= increment
                if remaining <= 1:
                    break
        distributions[border_territories[0]] += remaining
...
def handle_attack(game: Game, bot_state: BotState, query: QueryAttack) -> Union[MoveAttack, MoveAttackPass]:
    ...
    elif len(game.state.recording) < 4000:
        enemy = None
        new_records = game.state.recording[game.state.new_records:]
        for record in new_records:
            match record:
...
def handle_troops_after_attack(game: Game, bot_state: BotState, query: QueryTroopsAfterAttack) -> MoveTroopsAfterAttack:
    ...
    if game.state.territories[move_attack.attacking_territory].troops < 5:
