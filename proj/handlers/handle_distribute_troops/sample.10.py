...
def handle_distribute_troops(game: Game, bot_state: BotState, query: QueryDistributeTroops, mapNetwork: MapNetwork) -> MoveDistributeTroops:
    ...
    if avg <=30 or game_state < 300 and domination <=3:
        ...
        # enemy_nodes = set()
        # for i in border_to_base:
        #     for j in (set(mapNetwork.get_neighbors(i))-set(my_territories)):
        #             enemy_nodes.add(j)

        # enemy_nodes = list(enemy_nodes)
        ...
        troops_per_territory = remaining_troops // len(border_to_base)
        leftover_troops = total_troops % len(border_to_base)
        for territory in border_to_base:
            distributions[territory] += troops_per_territory
    
        # The leftover troops will be put into border with most troops
        distributions[stationed_troops[0][0]] += leftover_troops
