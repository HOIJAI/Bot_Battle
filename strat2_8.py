...
#         count = 0
#         for i in continent_owned:
#             out = mapNetwork.find_max_troop_adjacent_node(i[0], i[1])
#             if out != None:
#                 my_closet_node, node_to_attack = out
...
#                 count +=1
#             if count >=2:#only attack at most two of these
#                 break
    
#     #2. around nexus, locate node with the greatest difference in forces on the same continent/near bridges (us > enemy)
#     # put more until troops num ≥ 2.5x of the opposite force if possible, if not, move on to offensive
#     else:
#     #3. check if my continent is secured(all borders are >= adjacent), if not, all in one node to dominate the rest of the continent
#     # if yes, use dijkstra to find shortest path to own another continent, and start stacking near the bridge (late game)
#         distributions[random.sample(my_territories, 1)[0]] += 1
#         total_troops -=1
